...
export const paymentsApi = {
  ...
  // Poll payment status (resolves from DB — updated by Airtel/MTN callbacks)
  checkStatus: (ref: string) => api.get<ApiResponse<{ reference: string; status: string }>>(`/payments/status/${ref}`),
  receipt:     (id: string) => api.get(`/payments/${id}/receipt`, { responseType: 'blob' }),
  /**
   * Poll until status is 'completed' or 'failed', or until maxAttempts is reached.
   * Useful after initiating an Airtel/MTN payment — keeps checking until callback fires.
   */
  pollStatus: async (
    ref: string,
    ...
    const maxAttempts  = options?.maxAttempts  ?? 20; // 20 × 3s = 60s max
    let attempts = 0;
    return new Promise((resolve) => {
      const check = async () => {
        attempts++;
        try {
          const res = await api.get<ApiResponse<{ reference: string; status: string }>>(`/payments/status/${ref}`);
          const result = res.data?.data ?? { reference: ref, status: 'pending' };
          ...
          if (attempts >= maxAttempts) resolve({ reference: ref, status: 'pending' });
